...
if(fileInput) fileInput.addEventListener("change", async ()=>{
  ...
  try{
    const audioExts = ["mp3","m4a","wav","ogg","webm","aac"];
    if(audioExts.includes(ext)){
      await transcribeAndAttach(file, file.name);
      return;
    }
...
// Nxjerr labels/vlera nga rreshtat e Excel-it — zbulon vetë kolonën e etiketave
// (p.sh. Viti) dhe kolonën e vlerës (p.sh. Paga), edhe nëse ka kolona
// "zhurmë" (kategori të përsëritura, muaj gjysmë bosh) para tyre.
function buildChartData(rows){
  ...
  for(let c = 0; c < numCols; c++){
    const vals = dataRows.map(r => r[c]).filter(v => v !== undefined && v !== "");
    const fillRatio = vals.length / dataRows.length;
    const uniqueRatio = vals.length ? new Set(vals.map(String)).size / vals.length : 0;
    const numericRatio = vals.length ? vals.filter(v => typeof v === "number").length / vals.length : 0;
    colStats.push({ c, fillRatio, uniqueRatio, numericRatio });
  }

  // Kolona e etiketave: mirë e mbushur + vlera kryesisht unike (p.sh. vite, emra)
  let labelCandidates = colStats.filter(s => s.fillRatio >= 0.85 && s.uniqueRatio >= 0.7);
  if(!labelCandidates.length) labelCandidates = colStats.filter(s => s.fillRatio >= 0.5);
  if(!labelCandidates.length) return null;
  const labelCol = [...labelCandidates].sort((a,b) => b.uniqueRatio - a.uniqueRatio)[0];

  // Kolona e vlerës: numerike, mirë e mbushur, sa më afër kolonës së etiketave
  const valueCandidates = colStats.filter(s => s.c !== labelCol.c && s.fillRatio >= 0.85 && s.numericRatio >= 0.7);
  if(!valueCandidates.length) return null;
  valueCandidates.sort((a,b) => Math.abs(a.c - labelCol.c) - Math.abs(b.c - labelCol.c));
  const valueCol = valueCandidates[0];

  const labels = dataRows.map(r => String(r[labelCol.c] ?? "")).slice(0, 30);
  const values = dataRows.map(r => typeof r[valueCol.c] === "number" ? r[valueCol.c] : 0).slice(0, 30);
  const valueLabel = headers[valueCol.c] || "Vlera";
...
async function transcribeAndAttach(blob, displayName){
  attachChip.classList.remove("hidden");
  attachName.textContent = "duke transkriptuar " + (displayName || "regjistrimin") + "…";
  try{
    const form = new FormData();
    form.append("file", blob, displayName || "recording.webm");
    ...
    const label = displayName || ("Regjistrim (" + recSeconds + "s)");
    attachedFile = { name: label, kind:"docx", text: transcript };
...
function exportPDF(text){
  try{
    ...
    doc.save("smokey-" + Date.now() + ".pdf");
  }catch(e){ alert("Gabim PDF: " + e.message); }
}
...
function exportWord(text){
  try{
    ...
    downloadBlob(blob, "smokey-" + Date.now() + ".docx");
  }catch(e){ alert("Gabim Word: " + e.message); }
}
...
function exportExcel(text){
  try{
    ...
    const ws = XLSX.utils.aoa_to_sheet(rows.length ? rows : [["(bosh)"]]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Smokey");
    XLSX.writeFile(wb, "smokey-" + Date.now() + ".xlsx");
  }catch(e){ alert("Gabim Excel: " + e.message); }
}

function renderExportBar(text){
  const bar = document.createElement("div");
  bar.className = "export-bar";
  const mk = (label, fn) => {
    const b = document.createElement("button");
    b.type = "button"; b.textContent = label;
    b.addEventListener("click", ()=> fn(text));
    bar.appendChild(b);
  };
  mk("📄 PDF", exportPDF);
  mk("📝 Word", exportWord);
  mk("📊 Excel", exportExcel);
